...
let speedone = new Image()
speedone.src = 'https://everux-beta.onrender.com/img/effect_hue.png'

let speedtwo = new Image()
speedtwo.src = 'https://everux-beta.onrender.com/img/speed_varientII.png'
		...
		return {
			...
			gui: () => {
				...
				if (indices.party) {
					gui.party = get.next()
					if (!host)
						location.hash = '#' + global.server.id
				}
			},
	...
	function drawPoly(context, centerX, centerY, radius, sides, angle = 0, fill = true) {
		...
		if (!sides) { // Circle
          ...
          context.save()
              ...
              context.drawImage(speedtwo, -radius*0.5*bruh, -radius*0.5*bruh, radius*bruh, radius*bruh)
        context.restore()
	 }  else if (sides == 9000) { // Speed I
              let bruh = 3;
          context.save()
              context.beginPath()
              // Let's avoid Zero divisions
              ...
              context.rotate(angle)
              //context.scale(radius*bruh, radius*bruh),
              context.drawImage(speedone, -radius*0.5*bruh, -radius*0.5*bruh, radius*bruh, radius*bruh)
        context.restore()
	...
	if (instance.drawsHealth) {
		...
		if (health < 1 || shield < 1) {
			...
			if (config.graphical.shieldbars) {
				...
				drawBar(x - size, x + size, yy, (3 + config.graphical.barChunk) * ratio, color.black);
				drawBar(x - size, x - size + 2 * size * health, yy, 4 * ratio, color.lgreen);
				if (shield) {
					ctx.globalAlpha = .3 + shield * .3;
					drawBar(x - size, x - size + 2 * size * shield, yy, 4 * ratio, color.teal);
					ctx.globalAlpha = 1
				}
			}
			ctx.globalAlpha = fade
...
const gameDraw = (() => {
    ...
    { // Draw messages
		...
		if (text.name === "") {
			if (name.length) namecolor = mixColors(color.guiwhite, namecolor, .125)
		}
		...
		if (global.mobile) { // Draw skill bars
			...
			if (global.canSkill) {
				gui.skills.forEach((skill, ticker) => {
					...
					let [skillNameTop, skillNameBottom] = skillNameParts.length === 1 ? [skillNameParts, null] : [skillNameParts.slice(0, skillNameCut),
